...
def group_list (request):    
        ...
        if request.method == 'POST':
                ...
                if localserializer.is_valid():
                        try:
                                res = group.objects.filter(id).last()
                                x = int (res.id)+1
                                print(x)
                        except :
                                x=1
                        ...
                        with transaction.atomic():
                                ...
                                try:
                                        ...
                                        if localserializer.is_valid():
                                                details = group.objects.filter(id = localrequest['id']).first()
                                ...
                                ModelMaster = group.objects.filter(id)
                                MasterSerializer = GroupSerializer (ModelMaster, many=True)
