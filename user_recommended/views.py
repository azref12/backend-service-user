...
def recommended_list (request):    
        ...
        if request.method == 'POST':
                ...
                if localserializer.is_valid():
                        try:
                                res = user_recommended.objects.filter(id).last()
                                x = int (res.id)+1
                                print(x)
                        except :
                                x=1
                        ...
                        with transaction.atomic():
                                ...
                                try:
                                        ...
                                        if localserializer.is_valid():
                                                users_recommended = user_recommended.objects.filter(id = localrequest['id']).first()

                                                recommendeds = user_recommended ( 
                                                                id = users_recommended,
                                ...
                                ModelMaster = user_recommended.objects.filter(id)
                                MasterSerializer = UserRecommendedSerializer (ModelMaster, many=True)
