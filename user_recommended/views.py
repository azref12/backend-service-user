...
class RecommendedList (generics.ListCreateAPIView) :
        queryset = user_recommended.objects.all()
        serializer_class = UserRecommendedSerializer
        DecodedGenerator = api_view
        permission_classes = [AllowAny]
        filter_backends = [SearchFilter, OrderingFilter]
        search_fields = ['id','user_recommended'] 

class RecommendedDetail (generics.RetrieveUpdateDestroyAPIView) :
        queryset = user_recommended.objects.all()
        serializer_class = UserRecommendedSerializer
        DecodedGenerator = api_view
        permission_classes = [AllowAny]
